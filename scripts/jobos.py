...
def doctor(*, check_browser: bool) -> int:
    ...
    try:
        ...
        with psycopg.connect(database_dsn(), connect_timeout=5) as conn, conn.cursor() as cur:
            ...
            cur.execute("SELECT EXISTS (SELECT 1 FROM schema_migrations WHERE migration_id = '069_resume_freshness_preflight.sql')")
            mark(results, "Migrations through 069", bool(cur.fetchone()[0]))
        ...
        mark(results, "Migrations through 069", False, "PostgreSQL unavailable")
    ...
    core = all(checks.get(name, False) for name in ("Python 3.11+", "Environment", "PostgreSQL", "Migrations through 069"))
